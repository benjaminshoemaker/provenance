...
describe("checkRateLimit", () => {
  ...
  it("should allow when count is below limit", async () => {
    mocks.mockExecute.mockResolvedValue({ rows: [{ count: 5 }] });
    const result = await checkRateLimit("user-1");
    expect(result.allowed).toBe(true);
  });

  it("should deny when count equals limit (20)", async () => {
    mocks.mockExecute.mockResolvedValue({ rows: [{ count: 20 }] });
    const result = await checkRateLimit("user-1");
    expect(result.allowed).toBe(false);
  });

  it("should deny when count exceeds limit", async () => {
    mocks.mockExecute.mockResolvedValue({ rows: [{ count: 25 }] });
    const result = await checkRateLimit("user-1");
    expect(result.allowed).toBe(false);
  });

  it("should allow at count 19 (one below limit)", async () => {
    mocks.mockExecute.mockResolvedValue({ rows: [{ count: 19 }] });
    const result = await checkRateLimit("user-1");
    expect(result.allowed).toBe(true);
  });

  it("should use a single atomic SQL call", async () => {
    mocks.mockExecute.mockResolvedValue({ rows: [{ count: 1 }] });
